...
The prompt in `tools/cxr-triage/triage.py` instructs the model to respond **only** with
a JSON object matching the schema — no preamble, no markdown fences.

The image is passed as a base64-encoded content block.

See [tools/cxr-triage/triage.py](../../tools/cxr-triage/triage.py) for the implementation.
