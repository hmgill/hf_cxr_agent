...
* **Model-driven** (``stage_overlay`` + ``build_orchestratorco_sandbox``): the
...
def overlay_capabilities(filesystem_tools: bool = True) -> list:
    """Capabilities for the model-driven SandboxAgent path.

    ``filesystem_tools`` controls the **Filesystem** capability, which exposes
    ``apply_patch`` — a *hosted* (freeform-grammar) ``CustomTool``:

    * ``True`` (default): full set ``[Filesystem(), Shell(), Compaction()]``.
      Use with models on the OpenAI **Responses API** (e.g. ``gpt-5.4-mini``),
      where hosted tools are supported and the agent can edit files with
      ``apply_patch``.
    * ``False``: ``[Shell(), Compaction()]`` only. Required for the **Chat
      Completions API** (e.g. Claude via LiteLLM), which rejects hosted tools
      ("Hosted tools are not supported with the ChatCompletions API"). The model
      authors/edits files through the shell instead (``ExecCommandTool`` is a
      plain ``FunctionTool``).

    ``build_orchestrator_sandbox`` picks the right value automatically from the
    model, so callers rarely set this directly.
    """
    caps: list = [Shell(), Compaction()]
    if filesystem_tools:
        caps.insert(0, Filesystem())
    return caps
...
def _coerce_payload(findings: Any) -> dict[str, Any]:
    ...
    Tolerated shapes:
      ...
      * Box-bearing "findings" (what the agent tends to emit): {"findings":
        [{"label","bbox":{x,y,w,h}}]} — promoted to "regions" below.
    """
    ...
    if not isinstance(data, dict):
        data = {}
    # The agent often passes boxes under "findings" with "label" (matching the
    # localize prompt's vocabulary) instead of "regions"/"finding". If there are
    # no regions but "findings" carries box-bearing dicts, use those as regions.
    # Plain-text findings (no bbox) are left alone so they stay text.
    if not data.get("regions"):
        f = data.get("findings")
        if isinstance(f, list) and any(
            isinstance(it, dict) and ("bbox" in it or {"x", "y", "w", "h"} <= set(it))
            for it in f
        ):
            data["regions"] = f
...
def _encode_overlay_png(image_path: str) -> tuple[bytes, int, int, int, int]:
    """Load + (optionally) downscale the CXR.

    Returns ``(png_bytes, disp_w, disp_h, orig_w, orig_h)`` where ``disp_*`` are
    the (possibly downscaled) staged-PNG dimensions and ``orig_*`` are the source
    image dimensions. Absolute pixel boxes are normalized against ``orig_*`` (the
    space the localizer reports in); ``disp_*`` are only the viewer's canvas size.
    ...
    if max(orig_w, orig_h) > OVERLAY_IMAGE_MAX_SIDE:
        scale = OVERLAY_IMAGE_MAX_SIDE / max(orig_w, orig_h)
        img = img.resize((int(orig_w * scale), int(orig_h * scale)), Image.LANCZOS)
    buf = io.BytesIO()
    img.save(buf, format="PNG")
    return buf.getvalue(), img.size[0], img.size[1], orig_w, orig_h
...
async def _stage_inputs(session: Any, image_path: str, findings: Any,
    ...
    png_bytes, png_w, png_h, orig_w, orig_h = _encode_overlay_png(image_path)

    # Absolute box coordinates are normalized against the *original* image
    # dimensions — that is the space the localizer reports in. The staged PNG may
    # be downscaled for size, but the displayed dims share the same aspect ratio,
    # so the resulting fractions place boxes correctly on the viewer canvas.
    # Priority: explicit payload dims → original image dims → staged-PNG dims.
    src_w = int(payload.get("image_width") or orig_w or png_w or 0)
    src_h = int(payload.get("image_height") or orig_h or png_h or 0)
